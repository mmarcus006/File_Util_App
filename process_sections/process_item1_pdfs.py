...
OUTPUT_DIR = BASE_DIR / "output" / "sections" / "item_1"
SCHEMA_PATH = BASE_DIR / "prompts" / "schemas" / "Item1_pydantic_schema.py"
SYSTEM_PROMPT_PATH = BASE_DIR / "prompts" / "system_prompts" / "Item_1_Intro_Prompt.md"
...
def already_processed(folder_id: str) -> bool:
    ...
    """
    output_file = OUTPUT_DIR / f"{folder_id}_item1.json"
...
def load_schema_model():
    ...
    return schema_module.FranchiseAndFDDInfo
...
def find_target_pdfs(folder_path: Path) -> Tuple[Optional[str], Optional[str]]:
    """Find the ITEM_1 and intro PDF files in the specified folder.
    
    Args:
        folder_path: Path to the folder to search
        
    Returns:
        Tuple[Optional[str], Optional[str]]: Paths to the item1 and intro files
    """
    item1_file = None
    intro_file = None
    ...
    for file in folder_path.glob("*.pdf"):
        ...
        # Exclude files with _ITEM10
        if "_ITEM_10" in filename:
            print(f"Skipping file (contains _ITEM10): {filename}")
            continue
            
        # Find ITEM_1 files - exact match for "ITEM_1.pdf"
        if "ITEM_1.pdf" in filename:
            print(f"Found ITEM_1 file: {filename}")
            item1_file = str(file)
        
        # Find intro files
        elif "intro" in filename:
            print(f"Found intro file: {filename}")
            intro_file = str(file)
    
    print(f"Results - ITEM_1 file: {item1_file}, Intro file: {intro_file}")
    return item1_file, intro_file
...
def process_subfolder(folder_id: str, folder_path: Path) -> None:
    ...
    item1_file, intro_file = find_target_pdfs(folder_path)
    
    # Skip if either file is missing
    if not item1_file or not intro_file:
    ...
    try:
        ...
        user_prompt = f"Extract information from this combined Item 1 and Introduction section for franchise {folder_id}"
    except Exception as e:
    ...
    try:
        ...
        save_structured_data_to_json(model_instance, str(OUTPUT_DIR), f"{folder_id}_item1.json")
